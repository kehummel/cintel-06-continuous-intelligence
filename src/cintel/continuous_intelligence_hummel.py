...
Paths (relative to repo root)
    ...
    OUTPUT FILE: artifacts/system_assessment_hummel.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.continuous_intelligence_hummel
"""
...
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "system_assessment_hummel.csv"
    ...
    if anomalies_df.height > 0:
        for row in anomalies_df.iter_rows(named=True):
            ...
            if row["error_rate"] > MAX_ERROR_RATE:
                reasons.append(f"error_rate={row['error_rate']:.4f} > {MAX_ERROR_RATE}")
            if row["avg_latency_ms"] > MAX_AVG_LATENCY:
                reasons.append(
                    f"avg_latency_ms={row['avg_latency_ms']:.2f} > {MAX_AVG_LATENCY}"
                )
            LOG.warning(f"ANOMALY | {'; '.join(reasons)}")
    else:
        LOG.info("STEP 3. No anomalies found — all signals within thresholds.")
